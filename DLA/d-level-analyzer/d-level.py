...
for entry in entrylist:
    ...
    for feature in features:
        ...
        if "ORTH " in feature:
            ...
            singular = ""
        elif str.find(feature, "SUBJECT") == 0:
            subject = 1
        elif str.find(feature, "OBJECT") == 0:
            object = 1
...
def isTopS(node, mother):
    if node in mother and getCat(mother[node]) == "TOP":
        return 1
    return 0
...
#        return 1
        ...
        while headwordCat[:2] != "NN" and newHead in rules and "~" in newHead:
...
def isNPGranD(head, mother):
    ...
    while getCat(npMother)[:2] == "NP" and npMother in mother:
        npMother = mother[npMother]
    if getCat(npMother) == "PP" and npMother in mother:
                ...
                for daughter in daughters:
                    ...
                    # 2c - conjoined adjectival construction: H14
                        ...
                        if daughters.index(daughter) < ccNodeIndex:
                            ...
                            postJJNode = 1
                if (preNNode and postNNode and (subjs[head] != "" or (head in mother and isNPGranD(head, mother) and subjs[isNPGranD(head, mother)] != ""))) or (preJJNode and postJJNode):
                    scores.append(2)
...
while line:
    ...
    for head in heads:
        ...
        if subjs[head] == "" and head[:1] in ["V", "S"] and head in mother:
            gMother = mother[head]
            while subjs[gMother] == "" and gMother in mother:
                gMother = mother[gMother]
            subjs[head] = subjs[gMother]

        # find obj of S and VPs
        if head[:1] == "V" and objs[head] != "" and head in mother:
            gMother = mother[head]
